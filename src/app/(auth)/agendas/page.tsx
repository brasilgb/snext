...
const Agendas = (props: Props) => {
    ...
    const handleAgendaSearch = (term: string) => {
        setSearchInput(term);
        if (term.length > 2) {
            const filteredData = agendasAll.filter((fn: any, i: number) => {
    ...
    const onDelete = useCallback(
        async (e: any) => {
            ...
            const response = await sosapi.delete(`/agendas/${deleteId}`, {
                headers: {
                    Authorization: `Bearer ${user.token}`,
                },
            });
            ...
            if (status === 200) {
                setTimeout(async () => {
                    const res = await sosapi.get(`agendas?page=${linkValue}`, {
                        headers: {
                            Authorization: `Bearer ${user.token}`,
                        },
                    });
        ...
        <ABoxAll>
            ...
            <ABoxContent>
                <ATable>
                    ...
                    {agendas.map((agenda: any, icl: number) => (
                        <ATr
                            key={icl}
                            line={icl % 2}
                            lineDeleted={
                                deleteEffect === agenda.id ? true : false
                            }
                        >
                            <ATd>{agenda.cliente.nome}</ATd>
                            <ATd>
                            ...
                            <ATd>
                                {moment(agenda.cadastro).format('DD/MM/YYYY')}
                            </ATd>
                            <ATd>
                        ...
                        <div className="">
                            ...
                            <div className="text-center p-2 flex-auto justify-center">
                                ...
                                <p className="text-sm text-gray-500 px-8">
                                    Quer excluir este agendamento? Este processo
